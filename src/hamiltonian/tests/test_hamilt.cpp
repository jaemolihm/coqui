...
TEST_CASE("aug_hks_set_fock", "[hamilt]") {
  ...
  double e_diag = 0.0, e_x = 0.0, e_x2 = 0.0, den = 0.0;
  if (mpi->node_comm.root()) {
    auto F = sF.local(); auto Fx = sFx.local(); auto Fx2 = sFx2.local(); auto H0 = sH0.local();
      ...
      for (long k = 0; k < nkpts_ibz; ++k)
        for (long i = 0; i < nbnd; ++i) {
          for (long j = 0; j < nbnd; ++j) {
            ...
            e_x    += std::norm(Fx(s,k,i,j) + H0(s,k,i,j) - F(s,k,i,j));
  ...
  app_log(2, "aug_hks_set_fock: ||set_fock - diag(eps_QE)||/||eps_QE||      = {}",
          std::sqrt(e_diag)/nrm);
  app_log(2, "aug_hks_set_fock: ||(F_noH0 + H0) - F||/||eps_QE||            = {}",
          std::sqrt(e_x)/nrm);
