...
void lr_hf::thc_lr_hf(const sArray_t<AF_t>& sDeltaDm_skij,
  ...
  // allocated at all. IBC / δV / npol > 1 fall through to the dense branch.
  if (compute_hartree and not compute_exchange and npol == 1 and
      ibc == nullptr and DeltaV_qPQ == nullptr) {
    thc_lr_hartree_diag(sDeltaDm_skij, sDeltaF_skij, thc, DeltaF_PQ_out, compute_xc);
    return;
  }
...
template<nda::MemoryArray AF_t>
void lr_hf::thc_lr_hartree_diag(const sArray_t<AF_t>& sDeltaDm_skij,
                                sArray_t<AF_t>& sDeltaF_skij,
                                THC_ERI auto& thc,
                                nda::array<ComplexType, 4>* DeltaF_PQ_out,
                                bool compute_xc) {
  ...
  // (1) aux-basis density diagonal, averaged over the full BZ.
  ...
  long nsk = ns * nkpts;
  lr_thc_comm::primary_to_aux_diagonal(0, 0, sDeltaDm_skij.local(),
                                       nda::range(std::min<long>(_mpi->comm.rank(), nsk), nsk, np),
  ...
  double factor = (ns == 1 and _npol == 1) ? 2.0 : 1.0;
  ...
  // (3) ΔF_ij(k) = Σ_P conj(X(k+q)_Pi) ΔJ_P X(k)_Pj, one (s,k) block per rank.
  //     Each block is written by exactly one rank, so the closing reduction sums a
  //     value with zeros and all_reduce_parallel is bit-identical to all_reduce.
  _Timer.start("AUX_TO_PRIM");
  auto& F_comm = *sDeltaF_skij.communicator();
  sDeltaF_skij.win().fence();
  auto DeltaF_loc = sDeltaF_skij.local();
  long nsk_ibz = ns * nkpts_ibz;
  lr_thc_comm::diagonal_to_primary(0, 0, ComplexType(1.0), DeltaJ_PP, DeltaF_loc,
                                   nda::range(std::min<long>(F_comm.rank(), nsk_ibz),
                                              nsk_ibz, F_comm.size()),
                                   thc, _MF->ks_to_k(0), _kpq_map);
  sDeltaF_skij.win().fence();
  sDeltaF_skij.all_reduce_parallel();
  _Timer.stop("AUX_TO_PRIM");

  // Root-only ΔF_PQ for the Python phonon post-processors. Built directly from the
  // diagonal instead of gathering a distributed (ns, nk_ibz, NP, NP) array.
  _Timer.start("MISC");
