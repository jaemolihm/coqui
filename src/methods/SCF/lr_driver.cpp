...
std::tuple<int, double> lr_driver::run_lr(
  ...
  // estimate already lists: the two per-channel buffers, allocated only when
  // both channels carry a Σ.
...
void lr_driver::print_memory_estimate(long NP, bool include_gw_sigma, bool gw_full,
  ...
  if (include_gw_sigma) {
    ...
    // Split-kernel per-channel ΔΣ buffers on top of the (total, previous) pair.
    // Present (as a pair) only when both channels carry a Σ, i.e. pGW_GW0.
