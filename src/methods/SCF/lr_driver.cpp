...
lr_driver::lr_driver(simple_dyson& dyson, nda::array<double, 1> const& q_vec)
  ...
  for (auto& v : {"LR_SCF", "LR_DRIVER_SETUP",
                  ...
                  "LR_DYSON", "LR_HF", "LR_GW_SIGMA", "LR_GW_DW_TRANSPOSE",
                   ...
                   "LR_HF_PERT", "LR_GW_SIGMA_PERT", "LR_GW_DW_TRANSPOSE_PERT",
...
std::tuple<int, double> lr_driver::run_lr(
  ...
  if (include_gw_sigma) {
    ...
    utils::check(dW_qtPQ != nullptr && eps_inv_head != nullptr,
                 "lr_driver::run_lr: a GW self-energy is active but dW_qtPQ or eps_inv_head is null.");
  }
  ...
  // ΔΣ-sized shared arrays on top of the (total, previous) pair the base
  // estimate already lists.
  ...
  auto sDeltaSigma_prev_tskij = has_Sigma_sc
  ...
  struct sigma_clocks { const char *pi, *w, *sigma, *transpose; };
  ...
  constexpr sigma_clocks pert_clocks{"LR_GW_PI_PERT", "LR_GW_W_PERT",
                                     "LR_GW_SIGMA_PERT", "LR_GW_DW_TRANSPOSE_PERT"};
  ...
  auto eval_sigma_channel = [&](lr_kernel_spec const& mask,
    ...
    if (split_sigma_terms) {
      ...
      gw_solver.evaluate_sigma_DeltaW(
          sSigma_out, sG_tskij.local(), dDeltaW_qtPQ, thc,
          *opt_dG_tsRPQ, *opt_dG_mtau_tsRPQ);
      if (div_corr && is_q_gamma()) {
        gw_solver.apply_div_correction_G(
            sSigma_out, sG_tskij.local(), sS_skij.local(), thc, delta_eps_inv_head);
  ...
  // epilogue does: each node rank wrote its own slice, and each node computed
  // its own DIIS coefficients from its own all_reduce, so the node replicas
  // must be re-synchronized from node 0.
  auto outer_sync = [&](auto*... arrs) {
    ...
    if (_mpi->node_comm.root()) {
      auto bcast = [&](auto* p) {
        if (p) _mpi->internode_comm.broadcast_n(p->local().data(), p->local().size(), 0);
      };
      (bcast(arrs), ...);
    }
    (fence(arrs), ...);
  ...
  for (iter = 1; iter <= max_iter; ++iter) {
    ...
    if (_mpi->node_comm.root() && stage_iter > 1) {
      sDeltaDm_prev_skij.local() = sDeltaDm_skij.local();
      sDeltaF_prev_skij.local() = sDeltaF_sc_skij.local();
      if (has_Vcorr) {
        sDeltaVcorr_prev_skij.local() = sDeltaVcorr_skij.local();
      } else if (has_Sigma_sc) {
        sDeltaSigma_prev_tskij.local() = pDeltaSigma_sc->local();
      }
    }
    _mpi->comm.barrier();
    _Timer.stop("LR_SAVE");
    ...
    if (stage_iter > 1 && (sc_hf || sc_sigma || has_Vcorr)) {
      ...
      if (use_diis) {
        ...
        if (has_Vcorr) {
          _lr_diis->next_step_combined(
              _mpi->node_comm,
              sDeltaF_sc_skij.local(), sDeltaF_prev_skij.local(),
              sDeltaVcorr_skij.local(), sDeltaVcorr_prev_skij.local(), stage_iter);
        } else if (has_Sigma_sc) {
          _lr_diis->next_step_combined(
              _mpi->node_comm,
              sDeltaF_sc_skij.local(), sDeltaF_prev_skij.local(),
              pDeltaSigma_sc->local(), sDeltaSigma_prev_tskij.local(), stage_iter);
        } else {
          nda::array<ComplexType, 5> empty_sigma;
          _lr_diis->next_step_combined(
              _mpi->node_comm,
              sDeltaF_sc_skij.local(), sDeltaF_prev_skij.local(),
              empty_sigma, empty_sigma, stage_iter);
        }
      } else if (mixing < 1.0 && _mpi->node_comm.root()) {
        sDeltaF_sc_skij.local() = mixing * sDeltaF_sc_skij.local()
        ...
        if (has_Vcorr) {
          ...
          pDeltaSigma_sc->local() = mixing * pDeltaSigma_sc->local()
      ...
      if (_mpi->node_comm.root()) {
        _mpi->internode_comm.broadcast_n(sDeltaF_sc_skij.local().data(),
                                         sDeltaF_sc_skij.local().size(), 0);
        if (has_Vcorr) {
          _mpi->internode_comm.broadcast_n(sDeltaVcorr_skij.local().data(),
                                           sDeltaVcorr_skij.local().size(), 0);
        } else if (has_Sigma_sc) {
          _mpi->internode_comm.broadcast_n(pDeltaSigma_sc->local().data(),
                                           pDeltaSigma_sc->local().size(), 0);
        }
      }
    ...
    auto norms_F = utils::lr_distributed_norm(
        _mpi->node_comm, sDeltaF_sc_skij.local(), sDeltaF_prev_skij.local(), stage_iter > 1);
    ...
    if (has_Vcorr) {
      auto norms_V = utils::lr_distributed_norm(
          _mpi->node_comm, sDeltaVcorr_skij.local(), sDeltaVcorr_prev_skij.local(), stage_iter > 1);
      ...
      auto norms_Sigma = utils::lr_distributed_norm(
          _mpi->node_comm, pDeltaSigma_sc->local(), sDeltaSigma_prev_tskij.local(), stage_iter > 1);
        ...
        if (outer_diis_on) {
          ...
          if (pert_hf && pert_sigma) {
            outer_diis->next_step_combined(
                _mpi->node_comm,
                sDeltaF_pert_skij.local(), opt_sDeltaF_pert_prev->local(),
                pDeltaSigma_pert->local(), opt_sDeltaSigma_pert_prev->local(),
                outer_step);
          } else if (pert_sigma) {
            ...
            outer_diis->next_step_combined(
                _mpi->node_comm,
                pDeltaSigma_pert->local(), opt_sDeltaSigma_pert_prev->local(),
                empty_slot, empty_slot, outer_step);
          } else {
...
void lr_driver::print_memory_estimate(long NP, bool include_gw_sigma, bool gw_full,
  ...
  if (include_gw_sigma) {
    ...
    // source, on top of the (total, previous) pair.
  ...
  // node_comm, so this is a per-node total spread across the node's ranks —
  // the same accounting as the node-replicated shared arrays.
