...
namespace methods {
  ...
  void pproc_t::analyt_cont(mf::MF &mf, analyt_cont::ac_context_t &ac_params, std::string dataset) {
    ...
    auto dA_iw_ski = make_distributed_array<local_Array_4D_t>(_context.comm, pgrid, {niw, ns, nkpts_ibz, nbnds});
    if(dataset != "DOS" and dataset != "A") { // backward compatibility with "A" name for spectral function
      ...
      auto dA_tau_ski = make_distributed_array<local_Array_4D_t>(_context.comm, pgrid, {ntau, ns, nkpts_ibz, nbnds});
    ...
    auto dA_w_ski = make_distributed_array<local_Array_4D_t>(_context.comm, pgrid,
                                                             {ac_params.Nw, ns, nkpts_ibz, nbnds});
  ...
  void pproc_t::spectral_interpolation(mf::MF &mf, ptree const& pt,
    ...
    using math::nda::make_distributed_array;
    using local_Array_4D_t = nda::array<ComplexType, 4>;
    auto dG_wska = make_distributed_array<local_Array_4D_t>(_context.comm, pgrid, {niw, ns, nkpts, nbnd});
    ...
    auto dA_wska = make_distributed_array<local_Array_4D_t>(_context.comm, pgrid, {ac_params.Nw, ns, nkpts, nbnd});
  ...
  auto pproc_t::evaluate_GS_diag(memory::darray_t<local_Array_5D_t, communicator_t> & dG_tau_skij,
    ...
    using math::nda::make_distributed_array;
...

      {
          ...
          auto dG_ij = make_distributed_array<local_Array_2D_t>(k_intra_comm, {k_intra_comm.size(), 1}, {nbnds, nbnds});
          auto dS_ij = make_distributed_array<local_Array_2D_t>(k_intra_comm, {k_intra_comm.size(), 1}, {nbnds, nbnds});
          auto dGS_ij = make_distributed_array<local_Array_2D_t>(k_intra_comm, {k_intra_comm.size(), 1}, {nbnds, nbnds});
      ...
      auto buffer2 = make_distributed_array<local_Array_5D_t>(_context.comm, {ntpools, 1, np/ntpools, 1, 1}, dG_tau_skij.global_shape());

      math::nda::redistribute(buffer1, buffer2);

      auto buffer2_diag = make_distributed_array<local_Array_4D_t>(_context.comm, {ntpools, 1, np/ntpools, 1}, {ntau, ns, nkpts, nbnds});
      ...
      auto GS_diag = make_distributed_array<local_Array_4D_t>(_context.comm, {1, 1, nkpools, np/nkpools}, {ntau, ns, nkpts, nbnds});
