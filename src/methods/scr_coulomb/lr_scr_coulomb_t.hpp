...
namespace solvers {
  ...
  public:
     ...
     *                    on the q-dist distribution
     *                    (scr_coulomb_fourier_t::ft_buffer_dist).
     ...
     *                          (scr_coulomb_fourier_t::ft_buffer_dist), not consumed
     ...
     *
     * All operands must be on the q-dist distribution
     * (scr_coulomb_fourier_t::ft_buffer_dist) — same processor grid and same (P, Q)
     ...
     * @param dW_full_wqPQ     - [IN] W_full(q) = W_c(iω) + V on the q-dist distribution
