...
TEST_CASE("redistribute_chunked_equivalence", "[math]")
{
  ...
  auto C = make_distributed_array<larray>(world, {1,1,size}, gshape);
  fill(C);
  auto D_ref = make_distributed_array<larray>(world, {size,1,1}, gshape);
  redistribute_alltoallv(C, D_ref, ComplexType(1.0), ComplexType(0.0), size_t(1) << 40);
  auto D = make_distributed_array<larray>(world, {size,1,1}, gshape);
